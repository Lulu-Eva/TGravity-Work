...
- 用户允许生成 HTML/GSAP 科技画布，或正在执行 `produce` 生产命令。
...
第一版必须是单文件 HTML，所有 CSS 内联，GSAP 使用 CDN，同时提供无 GSAP 的本地 fallback。它是 MP4 渲染源，不是最终交付物本身。生成命令：
...
- 必须暴露 `window.__tgravitySetTime(seconds)`，供本地渲染器逐帧设置时间。
