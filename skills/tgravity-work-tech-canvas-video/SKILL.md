---
name: tgravity-work-tech-canvas-video
description: TGravity Work 科技画布视频生产 Skill。用于把已剪好气口的人像视频和逐字稿或可本地转录音轨，生成按时间同步变化的 HTML/GSAP 科技信息画布，并渲染、验收可交付的背景动画 MP4，供后续在剪映中与人像视频合成。触发方式：科技画布、Codex剪视频、脚本驱动剪辑、HyperFrames背景动画、HTML/GSAP动画、背景动画MP4、可交付MP4、/tgravity-tech-video-produce、/tgravity-tech-video-render、/tgravity-tech-video-validate。
---

# TGravity Work Tech Canvas Video

你是 TGravity Work 的科技画布背景动画 MP4 生产入口。
...
- OpenMontage 只能作为架构参考。遇到 OpenMontage、一键成片、从调研到成片等需求，先读 `references/openmontage-patterns.md`，再把能力边界收敛到背景动画 MP4 生产。
...
| `/tgravity-tech-video-produce`、一口气生成、生产背景 MP4、跑完整链路 | 读取 `references/workflow.md` 和 `references/validation.md`，执行 `scripts/tech_canvas_pipeline.py produce --workspace "<项目根目录>" --style "<style_id>"` |
...
| `/tgravity-tech-video-check`、检查科技画布视频环境 | 读取 `references/config.md`，执行 `scripts/tech_canvas_pipeline.py check --json` |
| `/tgravity-tech-video-init`、初始化科技画布视频项目 | 读取 `references/input-contract.md`，执行 `scripts/tech_canvas_pipeline.py init --workspace "<项目根目录>"` |
| `/tgravity-tech-video-plan`、生成剪辑方案、生成 timeline | 读取 `references/workflow.md`，执行 `scripts/tech_canvas_pipeline.py plan --workspace "<项目根目录>"` |
| `/tgravity-tech-video-overview`、生成风格总览站、生成 6 种风格 | 读取 `references/style-presets.md`，执行 `scripts/tech_canvas_pipeline.py overview --workspace "<项目根目录>"` |
| `/tgravity-tech-video-hyperframes`、生成 HTML/GSAP 科技画布 | 读取 `references/hyperframes-rules.md`，把用户选择映射为 `style_id`，执行 `scripts/tech_canvas_pipeline.py hyperframes --workspace "<项目根目录>" --style "<style_id>"` |
| `/tgravity-tech-video-render`、渲染背景动画 MP4 | 读取 `references/validation.md`，确认 `hyperframes/index.html` 存在，再执行 `scripts/tech_canvas_pipeline.py render --workspace "<项目根目录>"` |
| `/tgravity-tech-video-validate`、验收背景动画、检查输出物 | 读取 `references/validation.md`，执行 `scripts/tech_canvas_pipeline.py validate --workspace "<项目根目录>" --require-source-video --require-production-manifest` |
...
- 不把 OpenMontage 的能力写成本 Skill 已支持能力；没有实现 research、TTS、素材生成、音乐、字幕、人像合成前，不得承诺“一键完整成片”。
- 不自动发布到小红书、抖音、视频号。
