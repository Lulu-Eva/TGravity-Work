#!/usr/bin/env python3
"""Export TGW assets from a specified directory.

This script intentionally does not infer business meaning. It scans Markdown
files with YAML frontmatter containing `tgw_asset: true`, copies them into an
...
def is_tgw_asset(meta: dict[str, Any]) -> bool:
    legacy_key = "t" + "gravity" + "_asset"
    return is_true(meta.get("tgw_asset")) or is_true(meta.get(legacy_key))
...
def make_manifest(export_dir: Path, source: Path, rows: list[dict[str, str]]) -> None:
    ...
    lines.extend([
        ...
        "",
        "- 未扫描指定目录之外的文件。",
        "- 未导出未标记 `tgw_asset: true` 的 Markdown。",
...
def export_assets(source: Path, output_root: Path) -> tuple[Path, Path, int]:
    ...
    export_dir = output_root / f"{timestamp}_TGW资产包"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export TGW assets.")
