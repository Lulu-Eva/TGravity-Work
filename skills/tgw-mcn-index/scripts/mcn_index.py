#!/usr/bin/env python3
"""Build and validate TGW MCN relationship indexes."""
...
def build_indexes(mcn_root: Path, fill_links: bool = False, dry_run: bool = False, backup: bool = True) -> dict[str, Any]:
    ...
    for file_name in INDEX_FILES.values():
        summary_lines.append(f"- `tgw-data/mcn/indexes/{file_name}`")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build TGW MCN relationship indexes.")
    parser.add_argument("--root", help="Project root containing tgw-data/mcn.")
    ...
    if args.mcn_root:
        ...
        mcn_root = Path(args.root).expanduser().resolve() / "tgw-data" / "mcn"
    else:
        mcn_root = Path.cwd() / "tgw-data" / "mcn"
