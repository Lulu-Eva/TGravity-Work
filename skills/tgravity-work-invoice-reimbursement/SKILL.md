---
name: tgravity-work-invoice-reimbursement
description: TGravity Work 发票报销整理 Skill。用于整理发票、发票报销、文本型 PDF 发票识别、高铁票报销、差旅发票、按模板填写发票、发票下载目录生成 Excel 报销表。递归扫描可抽取文本的 PDF 发票和铁路电子客票，按默认 6 列输出，自动按发票号码去重，整理完成后自动执行复查，重点区分客运服务费、代订机票费、铁路电子客票和普通电子发票；扫描件、图片发票和原生 OFD 先列为待处理，不硬猜。
---

# TGravity Work Invoice Reimbursement

你是 TGravity Work 的发票识别与报销整理入口。
