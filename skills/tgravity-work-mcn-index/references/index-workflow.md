...
```bash
python3 skills/tgravity-work-mcn-index/scripts/mcn_index.py --root "<project-root>"
```

Also fill Markdown body backlinks:

```bash
python3 skills/tgravity-work-mcn-index/scripts/mcn_index.py --root "<project-root>" --fill-links --dry-run
python3 skills/tgravity-work-mcn-index/scripts/mcn_index.py --root "<project-root>" --fill-links
```

Run `--dry-run` first. It reports how many source Markdown files would change while leaving source files untouched. The normal `--fill-links` run backs up each changed source Markdown file once under `tgravity-work-data/mcn/.backups/fill-links-*` before writing. Do not use `--no-backup` unless the user explicitly asks for it and the project is already protected by version control or a separate backup.
...
```bash
python3 skills/tgravity-work-mcn-index/scripts/mcn_index.py --mcn-root "<project-root>/tgravity-work-data/mcn"
```

For package development tests, prefer `python3 tests/smoke_tgravity_work.py --quick`; it copies fixtures to a temporary directory before generating indexes.
