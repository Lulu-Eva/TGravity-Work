#!/usr/bin/env python3
"""TGW Search: Perplexity + Tavily dual-engine search."""
...
CONFIG_DIR = Path(os.environ.get("TGW_SEARCH_HOME", Path.home() / ".config" / "tgw-search"))
CONFIG_FILE = CONFIG_DIR / "config.json"
LEGACY_CONFIG_FILE = Path.home() / ".config" / ("t" + "gravity" + "-work-search") / "config.json"
VERSION = "0.1.0"
    ...
    try:
        resp = requests.post(
            ...
            headers={
                ...
                "User-Agent": f"tgw-search/{VERSION}",
            },
            json={
                "model": model,
...
def tavily_search(query: str, search_depth: str, max_results: int, timeout: int) -> dict[str, Any]:
    ...
    try:
        resp = requests.post(
            ...
            headers={"Content-Type": "application/json", "User-Agent": f"tgw-search/{VERSION}"},
            json={
                "api_key": api_key,
                "query": query,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run TGW Search through Perplexity + Tavily.")
