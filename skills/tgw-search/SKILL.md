---
name: tgw-search
description: TGW-Work-Search：词元引力双引擎公开网页搜索与资料核验 Skill。用于通过 Perplexity + Tavily 并行搜索，做近期信息核验、竞品/品牌/达人/平台公开资料调研、带来源链接的检索结果整理。首次使用需要本机配置 Perplexity API Key 和 Tavily API Key；公开 GitHub 仓库不得包含 API 密钥。只在用户明确说“搜索技能”、/TGW-search、/TGW-search-init、/TGW-search-status、TGW搜索，或 TGW 主 Skill 明确路由到搜索时使用。
---
...
- 本 Skill 使用一个专门的本机 API Key 配置文件：`~/.config/tgw-search/config.json`。后续搜索优先从该文件读取 key；该文件只在员工当前电脑上，不属于 GitHub 仓库。
...
| 用户信号 | 动作 |
|---|---|
| `搜索技能`、`/TGW-search`、TGW搜索、TGW 主 Skill 明确要求搜索 | 先执行 `scripts/dual_search.py --status`；`missing` 时进入初始化，`partial` 时说明缺失项但继续搜索，`configured` 时直接搜索 |
| `/TGW-search-init`、配置搜索、初始化搜索、设置搜索 API Key、设置 Perplexity API Key、设置 Tavily API Key | 读取 `references/search-config.md`；用户已在对话中提供 key 时必须使用 `scripts/dual_search.py --init-stdin` 写入本机配置文件，用户未提供 key 且终端可交互时才使用 `scripts/dual_search.py --init` |
| `/TGW-search-status`、检查搜索配置、搜索能用吗 | 执行 `scripts/dual_search.py --status` |
| 用户要求保存搜索结果 | 先生成草稿；用户确认后保存到运行项目的 `tgw-search-data/`，不要保存到 Skill 仓库 |
...
这类任务回到 TGW 的 `/TGW-workcheck` 做人机分工。
