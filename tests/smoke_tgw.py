#!/usr/bin/env python3
"""Semantic smoke tests for the TGW skill package."""
...
ROUTER_SKILL = ROOT / "skills" / "tgw" / "SKILL.md"
ONBOARDING_SKILL = ROOT / "skills" / "tgw-onboarding" / "SKILL.md"
GOAL_SKILL = ROOT / "skills" / "tgw-goal" / "SKILL.md"
ASSET_CARDS_SKILL = ROOT / "skills" / "tgw-asset-cards" / "SKILL.md"
MCN_ROUTER_SKILL = ROOT / "skills" / "tgw-mcn" / "SKILL.md"
PROMPT_OPTIMIZER_SKILL = ROOT / "skills" / "tgw-prompt-optimizer" / "SKILL.md"
PROMPT_ARCHITECT_SKILL = ROOT / "skills" / "tgw-prompt-architect" / "SKILL.md"
PIPELINE = ROOT / "skills" / "tgw-tech-canvas-video" / "scripts" / "tech_canvas_pipeline.py"
VIDEO_PIPELINE = ROOT / "skills" / "tgw-video-indexer" / "scripts" / "video_pipeline.py"
SEARCH_SCRIPT = ROOT / "skills" / "tgw-search" / "scripts" / "dual_search.py"
INVOICE_SCRIPT = ROOT / "skills" / "tgw-invoice-reimbursement" / "scripts" / "invoice_reimbursement.py"
VALIDATE_ASSET_CARDS = ROOT / "skills" / "tgw-asset-export" / "scripts" / "validate_asset_cards.py"
EXPORT_ASSETS = ROOT / "skills" / "tgw-asset-export" / "scripts" / "export_assets.py"
MCN_INDEX = ROOT / "skills" / "tgw-mcn-index" / "scripts" / "mcn_index.py"
MCN_RENDER_SHARED = ROOT / "shared" / "mcn" / "render_mcn_asset.py"
MCN_SYNC_SHARED = ROOT / "shared" / "mcn" / "sync_shared_resources.py"
MCN_RENDER = ROOT / "skills" / "tgw-mcn-creator-profile" / "scripts" / "render_mcn_asset.py"
...
def assert_tgw_trigger_contract() -> None:
    scanned = [
        ...
        *sorted((ROOT / "skills").glob("*/agents/openai.yaml")),
        ROOT / "skills" / "tgw-onboarding" / "assets" / "templates" / "onboarding-checklist.md",
        ROOT / "skills" / "tgw-profile" / "assets" / "templates" / "user-profile.md",
        ROOT / "skills" / "tgw-search" / "references" / "search-config.md",
    ]
    ...
    for path in scanned:
        if not path.exists():
            continue
        text = path.read_text(encoding="utf-8")
        old_slash_pattern = r"/" + "t" + r"gravity[-A-Za-z0-9]*"
        for match in re.finditer(old_slash_pattern, text):
            token = match.group(0)
            ...
            if token == "/tgw" and "skills/tgw" in line:
                continue
...
def assert_mcn_skill_contract() -> None:
    ...
    mcn_skill_dirs = [
        ROOT / "skills" / "tgw-mcn-creator-profile",
        ROOT / "skills" / "tgw-mcn-brand-profile",
        ROOT / "skills" / "tgw-mcn-brief-builder",
        ROOT / "skills" / "tgw-mcn-collaboration",
        ROOT / "skills" / "tgw-mcn-index",
    ]
...
def copy_mcn_fixture(workspace: Path) -> Path:
    mcn_root = workspace / "tgw-data" / "mcn"
...
def assert_mcn_index_smoke() -> None:
    with tempfile.TemporaryDirectory(prefix="tgw-mcn-smoke-") as tmp:
        workspace = Path(tmp)
        ...
        index_root = workspace / "tgw-data" / "mcn" / "indexes"
        required_files = [
            "关系索引.csv",
            "关系总览.md",
            "品牌-达人矩阵.csv",
        ...
        brand_file = workspace / "tgw-data" / "mcn" / "brands" / "BRD-20260627-001_测试品牌.md"
        creator_file = workspace / "tgw-data" / "mcn" / "creators" / "CRT-20260627-001_测试达人.md"
...
def assert_mcn_index_dry_run_smoke() -> None:
    with tempfile.TemporaryDirectory(prefix="tgw-mcn-dry-run-") as tmp:
        workspace = Path(tmp)
        mcn_root = copy_mcn_fixture(workspace)
...
def assert_mcn_invalid_asset_smoke() -> None:
    with tempfile.TemporaryDirectory(prefix="tgw-mcn-invalid-") as tmp:
        workspace = Path(tmp)
        mcn_root = copy_mcn_fixture(workspace)
...
def assert_mcn_generated_only_relationship_smoke() -> None:
    with tempfile.TemporaryDirectory(prefix="tgw-mcn-generated-only-") as tmp:
        workspace = Path(tmp)
        mcn_root = copy_mcn_fixture(workspace)
...
def assert_mcn_render_smoke() -> None:
    with tempfile.TemporaryDirectory(prefix="tgw-mcn-render-") as tmp:
        workspace = Path(tmp)
        output_dir = workspace / "tgw-data" / "mcn" / "creators"
        command = [
            sys.executable,
            str(MCN_RENDER),
            "--template",
            str(ROOT / "skills" / "tgw-mcn-creator-profile" / "assets" / "templates" / "mcn-creator-profile.md"),
            "--output-dir",
            str(output_dir),
            "--asset-id",
            "CRT-20260627-099",
        ...
        bad_target = subprocess.run(
            [
                ...
                str(ROOT / "skills" / "tgw-mcn-creator-profile" / "assets" / "templates" / "mcn-creator-profile.md"),
                "--output-dir",
                str(ROOT / "skills" / "tgw-mcn-creator-profile" / "tgw-data" / "mcn" / "creators"),
                "--asset-id",
                "CRT-20260627-100",
                "--title",
                "禁止写入Skill包",
...
def assert_mcn_export_smoke() -> None:
    with tempfile.TemporaryDirectory(prefix="tgw-mcn-export-") as tmp:
        workspace = Path(tmp)
        mcn_root = copy_mcn_fixture(workspace)
...
def assert_srt_plan_smoke() -> None:
    with tempfile.TemporaryDirectory(prefix="tgw-srt-smoke-") as tmp:
        workspace = Path(tmp)
        write_srt(workspace)
...
def assert_script_failure_handling() -> None:
    search = load_module(SEARCH_SCRIPT, "tgw_search_smoke")
    ...
    video = load_module(VIDEO_PIPELINE, "tgw_video_smoke")
    ...
    invoice = load_module(INVOICE_SCRIPT, "tgw_invoice_smoke")
    with tempfile.TemporaryDirectory(prefix="tgw-invoice-failure-") as tmp:
...
def assert_produce_validate_smoke() -> None:
    if not local_render_available():
        ...
        return
    with tempfile.TemporaryDirectory(prefix="tgw-produce-smoke-") as tmp:
        workspace = Path(tmp)
        write_srt(workspace)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run TGW semantic smoke tests")
