...
TGravity Work Skills 是词元引力内部经营记录、资产交接、公开网页搜索和视频素材索引 Skill 包。它面向词元引力员工，帮助员工先判断一项工作的人机分工，再把日常口喷、品牌线索、达人信息、商单推进、复盘记录整理成可保存、可导出、可交接的 Markdown 资产；需要公开资料搜索时，单独使用 `tgravity-work-search`；需要整理大量视频素材和切片表时，单独使用 `tgravity-work-video-indexer`。
...
- `tgravity-work-video-indexer`：视频素材索引、逐字稿对齐、关键帧/contact sheet、低置信 ASR 补充和切片表导出。
...
`tgravity-work-video-indexer` 负责：

- 扫描用户指定项目目录里的视频素材，不默认扫描整台电脑。
- 用 `ffprobe` 提取时长、分辨率、音频轨、拍摄时间等元数据。
- 解析现成逐字稿、SRT 或 Whisper JSON，并按绝对时间对齐到视频。
- 用 `ffmpeg` 生成关键帧和 contact sheet，方便人工复核画面。
- 导出 CSV / Excel 版视频素材切片表，并标注低置信和待人工复核项。
...
```bash
npx skills add Lulu-Eva/TGravity-Work -g -y --skill tgravity-work-video-indexer
```
...
python3 ~/.codex/skills/.system/skill-installer/scripts/install-skill-from-github.py --repo Lulu-Eva/TGravity-Work --path skills/tgravity-work-video-indexer
```
...
新手教程不是固定脚本。它会根据员工反馈，在“完全新手、工作分配、日报生成、搜索技能、视频分析技能、人机边界”之间调整下一步。

如果员工说“继续下一课”，Skill 会优先接着当前对话继续；员工明确保存过学习进度时，才读取本机 `tgravity-work-data/learning/current-user.md`。

教程里的搜索和视频触发词只用于认识对应 Skill，不要求员工复制执行。真实搜索或真实配置 API Key 时，再单独说“搜索技能”，或启动 `/tgravity-search`、`/tgravity-search-status`。真实处理视频项目时，再单独说“视频分析技能”，或启动 `/tgravity-video`。
...
Skill 会先把工作定义清楚，再给出人类、Codex、WorkBuddy、Manus、TGravity Work Search、TGravity Work Video Indexer 的分工建议。
...
需要整理线下活动、访谈、课程、工作坊等大量视频素材时，说“视频分析技能”，也可以用 `/tgravity-video`。它会先确认项目目标和目录，再生成视频清单、逐字稿对齐结果、关键帧/contact sheet 和视频素材切片表。

这个 Skill 不替代最终剪辑判断。它负责把素材变成可复核表格；画面是否可用、哪段能发、是否涉及肖像/版权/隐私，仍然要人工判断。
...
视频 Skill 不需要 API Key。新电脑主要检查 `ffmpeg` / `ffprobe`：
...
```text
视频分析技能，项目目录是 /path/to/project。请先初始化视频素材工作区，然后生成素材切片表草稿。
```
...
python3 ~/.codex/skills/.system/skill-creator/scripts/quick_validate.py skills/tgravity-work-video-indexer
